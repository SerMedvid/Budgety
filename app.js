...
let UIController = (function(){
	...
	let formatNumber =  function(num, type) {
			...
			num = Math.abs(num);
			num = new Intl.NumberFormat('en-US', {maximumFractionDigits: 2, minimumFractionDigits: 2}).format(num);
	...
	return {
		...
		addListItem: function(obj, type){
			...
			//1. Create HTML string with plaeholder text
			switch(type) {
				case 'inc': 
...
let controller = (function(budgetCtrl, UICtrl){
	...
	let ctrlAddItem = function(){
		...
		if(input.description !== "" && !isNaN(input.value) && input.value > 0){
	 		...
	 		//4. Calculeate and update budget
	 		updateBudget();
